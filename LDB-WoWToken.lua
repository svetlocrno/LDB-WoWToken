...
local function InitSV(sv_name)
	if not _G[sv_name] then
		_G[sv_name] = {}
	end

	if not _G[sv_name].history_timestamp then
		_G[sv_name].history_timestamp = {}
	end
	if not _G[sv_name].history_price then
		_G[sv_name].history_price = {}
	end
end
InitSV(sv_name)
...
local function DiffString(now, before)
	...
	if diff < 1 then
		return color .. sign .. comma_value(diff / 1000000 * before) .. "|cfff0c80bg|r"
	else
		return color .. sign .. tonumber(string.format("%.1f", diff)) .. "%"
	end
end
...
local function OnTokenPriceUpdate(self, event, result)
	...
	if event == "_MANUAL_UPDATE" then
		...
		if result ~= LE_TOKEN_RESULT_SUCCESS then
			...
			new_price = GetCurrentMarketPrice()
		end
	end

	if new_price == current_price and current_h2 == history_price[2] then
		return
	end
	...
	if current_price then
		...
		local money_string = comma_value(current_price / 10000) .. "|cfff0c80bg|r"

		dataobj.text = money_string .. " " .. (diff or "")
		if LibToast and SV_LDBWoWToken.diff_libtoast ~= false then
			LibToast:Spawn(a_name, money_string, diff)
		end
		if SV_LDBWoWToken.diff_chat ~= false then
			print(a_name .. ": " .. money_string .. " " .. (diff or ""))
		end
	else
		dataobj.text = "N/A"
	end
...
function dataobj:OnEnter()
	...
	for idx = history_length, 1, -1 do
		...
		if price then
			...
			price = comma_value(price / 10000) .. "|cfff0c80bg|r" or ""
